...
class Grid {
    ...
    /**
     * Get the image or color at the specified column and row in the grid.
     * @param {number} x X-coordinate within the canvas.
     * @param {number} y Y-coordinate within the canvas.
     * @return {Picture | null} Returns the Picture object or null if no image is found.
     */
    getImageAt(x: number, y: number): Picture | null {
        if (!this._canvas || !this._context) {
            return null;
        }

        // Calculate the column and row based on x, y position
        const col = Math.floor(x / (this._width / this._columns));
        const row = Math.floor(y / (this._height / this._rows));

        // Get the color of the grid square at the specified column and row
        const color = this.getGridSquare(col, row);

        // Get the Picture object by the color
        return this.getPictureByColor(color);
    }
}
